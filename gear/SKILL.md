...
Initialize it with `hsh --initroot-only`. For a specific branch or architecture, select an existing matching `~/apt/apt.conf.<branch>.<arch>` and pass it to this initialization command. These APT configuration files were generated by [`gen-apt-files.sh`](https://github.com/Obirvalger/docker-brew-alt/blob/master/gen-apt-files.sh) from the `docker-brew-alt` repository. For example, to initialize a P11 AArch64 chroot:
